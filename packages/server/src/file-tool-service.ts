...
import { fileToolRequiresApproval, parseEventInput, parseFileToolInvocation, type Event, type EventInput } from "@fosil/contracts";
...
export interface FileToolServiceOptions { now?: () => Date; approvalTtlMs?: number }
...
export class FileToolService {
  ...
  constructor(private readonly store: SqliteWorkerStore, options: FileToolServiceOptions = {}) {
  ...
  prepare(sessionId: string, runId: string, providerCallId: string): Promise<string> {
    return this.coalesce(JSON.stringify(["prepare", sessionId, runId, providerCallId]), async () => {
      ...
      const gated = fileToolRequiresApproval(declared.name);
  ...
  private async advanceOnce(sessionId: string, runId: string, callId: string): Promise<ToolAdvance> {
    ...
    if (call.cwd !== state.workspaceRoot || call.requiresApproval !== fileToolRequiresApproval(call.toolName)) {
      throw new StoreError("policy_mismatch", "Recorded call does not match the file-tool policy or workspace");
    }
    ...
    let invocation;
    try { invocation = parseFileToolInvocation({ name: call.toolName, arguments: call.arguments }); }
    catch {
      return finish({ status: "failed", reason: "validation_failed", error: { code: "invalid_arguments", message: "Unknown file tool or invalid arguments", details: null } });
    }
    ...
    try {
      const executed = await executeFileTool(call.cwd, invocation, protectedFiles, async () => {
      ...
      if (error instanceof ToolCancelled) outcome = { status: "cancelled", reason: "cancel_requested" };
        ...
        outcome = {
          ...
          error: { code: error instanceof FileToolError ? error.code : "file_io", message: error instanceof FileToolError ? error.message : "File operation failed", details: null },
