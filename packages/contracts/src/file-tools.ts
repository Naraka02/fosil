...
export function fileToolDefinitions() {
  return [
    ...
    { name: "edit_file", description: "Replace an existing UTF-8 file only when its SHA-256 digest still matches. Read Only requires approval; workspace-write and full-access modes allow the managed in-workspace edit.", parameters: z.toJSONSchema(editArguments) }
  ];
}
