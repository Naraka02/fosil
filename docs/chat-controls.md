...
This reference owns the implemented browser Chat workflow, its event projection, mutation behavior, and current product limits. The [HTTP service](http-service.md) owns routes and browser-origin enforcement; the [event contracts](execution-events.md) own lifecycle facts; the [product scope](product-scope.md) owns release requirements beyond this slice. The [service and Web Agent Note](../.agents/notes/proposed/architecture/2026-08-27-execution-service-and-web.md#chat-controls-phase) records the phase decision.
...
The application is served only when the caller gives `ExecutionHttpServer` a valid browser build directory. The repository does not yet include a real-provider selector or a product launcher, so this interface currently runs through controlled test composition rather than a supported end-user startup command. Trace inspection is a separate unfinished slice.

## Saved-state projection

The client treats canonical events as its only conversation state. Selecting or reopening a session reads every page of one fixed history prefix, projects its runs, messages, model requests, tools, approvals, and terminal states, then opens EventSource strictly after the loaded sequence. Session list refreshes do not decide whether an approval is actionable or a run is complete.
...
The current UI does not expose complete model request context, reasoning, tool results, diffs, timings, token usage, retained-payload flags, recovery blocker resolution, session deletion, or export. Those facts remain available only through lower-level history where implemented and belong to the later Trace and product-completion slices. Configured-secret masking, retention budgets, real-provider acceptance, and hostile local-process isolation remain unfinished.
...
The browser test builds the application and uses real Chromium, loopback HTTP/SSE, the production agent loop, SQLite, a controlled provider, and an approved Shell fixture. It observes committed streaming text, refreshes before and after allow and deny decisions, proves the approved marker is written once and the denied marker is absent, cancels a waiting provider and observes its cleanup, reopens the cancelled run, checks a 390-pixel viewport for horizontal overflow, and refuses external resource requests. This evidence does not establish a real model, Trace completeness, sensitive-repository handling, or large-session performance.
