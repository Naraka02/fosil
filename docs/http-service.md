...
[ExecutionHttpServer](../packages/server/src/http/execution-http.ts) takes an already-open `SqliteWorkerStore`, `AgentLoopOptions`, an optional process-local provider credential controller, and an optional absolute `webRoot`. Construction checks that store opening and recovery completed, constructs its own loop, and opens no listening socket. When present, `webRoot` must resolve to a directory containing a regular `index.html` and `assets` directory; invalid or missing builds fail construction. `listen(port = 0)` binds only `127.0.0.1`; zero selects an ephemeral port. The returned HTTP origin includes the actual port. The [product launcher](deepseek-provider.md#product-launcher) supplies the built browser, DeepSeek model, database, masking, credential controller, and port configuration; the HTTP component itself does not discover workspaces.
...
| --- | --- |
| `GET /api/status` | Process-local `ready`, `failed`, or `stopping` status, launcher model identity, and non-secret API-key configured/source status |
| `GET /api/workspaces/directories` | Optional absolute Linux `path`; returns its canonical path, parent, and bounded child-directory names for the workspace picker |
| `POST /api/provider/credential` | An `api_key` from 8 through 16,384 characters; replaces the process-memory credential and returns only configured/source status |
| `POST /api/workspaces/delete` | Exact saved `workspace_root`; atomically deletes every idle, unblocked Fosil session under that root and returns deleted identities |
...
| `POST /api/sessions/:sessionId/delete` | Empty JSON object; atomically deletes one idle, unblocked Fosil session and returns its identity |
...
Directory discovery is a same-origin, read-only convenience for choosing an existing workspace. With no path it starts at the server process user's home directory. A requested directory is resolved to its canonical path, and the response contains its parent plus at most 500 real child directories sorted by name; regular files and symbolic-link entries are not returned, and `truncated` reports an omitted remainder. The operator may enter a more specific absolute path when the response is truncated or a desired path is a symbolic link. Missing, inaccessible, relative, file, malformed, and duplicate path queries return a bounded 400 error without failing the execution host. The route reads no file content, creates no directory, and does not replace the existing canonicalization and directory checks performed by `session.create`.

Deletion targets Fosil persistence only. A session deletion removes its index, events, payloads, session-scoped command receipts, and originating store-scoped creation receipt in one worker transaction. Workspace deletion selects sessions by exact saved canonical root and applies the same cleanup atomically to the complete group. Any active run or unresolved workspace outcome rejects the whole operation with 409. The route never removes, edits, traverses, or requires the continued existence of the local workspace directory. A lost deletion response may leave delivery uncertain; the browser performs no automatic retry and reconciles by reloading the session list.

Provider credential configuration is available only when the host receives an explicit controller. The host validates the key, registers its exact value with the store's in-memory masker, and then replaces the provider delegate without returning the value, a suffix, or a digest. The credential is absent from local storage, SQLite, commands, events, Trace, logs, and status payloads. Requests already in progress retain their snapshotted delegate; later requests use the replacement. A WebUI-supplied key is lost on process restart, while an environment-supplied key is restored by the launcher. This boundary does not hide the submitted value from the same-origin browser request itself or protect it from a hostile local process.

Malformed commands, cursors, and directory paths return 400, origin violations 403, unknown routes/sessions 404, unsupported methods 405, and command conflicts 409. Oversized requests return 413, non-JSON mutations 415, and exhausted session retained-payload capacity 507. Worker queue pressure, stream capacity, and unavailable service conditions return 503. Expected validation and admission conflicts do not by themselves fail the host. Raw lifecycle append operations, direct tool execution, private payload references, and database-file downloads are not HTTP endpoints.
