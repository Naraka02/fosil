...
A session binds the conversation to its recorded workspace; a run belongs to one accepted user command and projects its persisted approval mode; a step contains one ordinary model request and its requested tools. The optional `run.started.approval_mode` field accepts `manual`, `workspace_write`, or `full_access`; replay defaults an earlier event without the field to `manual`, so the version-1 store format remains readable. Request, call, and approval identities are tracked within their run and must retain their step/request correlation. An explicit attempt 1 `context_limit` failure followed by a successful `context_overflow` checkpoint permits exactly one new request identity as attempt 2 in the same step. No other retry is legal.
...
Within a run, model requests remain sequential. A complete model response can produce multiple tool calls: consecutive calls recorded as parallel may be active together, while exclusive calls require no active sibling and form ordering barriers. A later parallel call may start only after every earlier open sibling is itself parallel and running. Regardless of body completion order, terminal tool events must follow declaration order, so replay and model history remain deterministic. Stream fragments are recorded output and never authorize tool execution. A failed model request ends normal model dispatch unless it enters the single context-overflow recovery path; an ordinary tool failure can become context for another step.
...
Model input records contain the effective provider/model identity, system instructions, logical messages, tool schemas, call settings, optional Context Composition, and optional sanitized provider request metadata. The composition attributes local estimates and transformations to context sources; it does not replace the exact request or claim provider-tokenizer precision. This structured record is not a raw HTTP capture. Model output records retain returned text, provider-exposed reasoning, tool calls, stop reason, usage, timing, and optional sanitized provider response metadata. Tool records retain the operation context and its canonical result, error, exit code, and applicable evidence.

Compaction records form their own lifecycle. `context.compaction.started` identifies the source prefix, trigger, exact summarization request, Context Composition, and before/target measurements. Its terminal event records either a successful checkpoint with deterministic facts, exact shadowed event sequences, and tool-result transformations, or a bounded failure. The new fields are optional for compatibility with earlier version-1 events. The reducer permits no simultaneous provider request, tool, or compaction and does not treat generated summary prose as replacement canonical history. The [context compaction reference](context-compaction.md) owns model projection and recovery semantics.
...
Payload validation does not identify secrets, prove that a snapshot is complete, or establish that reported tool evidence matches real filesystem effects. The [file-tool service](file-tools.md#managed-mutation-and-evidence) produces bounded managed-mutation evidence. Producers remain responsible for those facts and for the [product data boundary](product-scope.md#data-boundary). Workspace path validation remains a syntax check, not filesystem admission or authorization.
