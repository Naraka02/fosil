...
This reference owns the implemented system composition and dependency boundaries. The repository contains shared contracts, pure state reduction, durable event storage and command acceptance, an immutable built-in tool registry, approved repository/file and shell tools, a provider-neutral agent loop with bounded safe sibling scheduling, request-level Context Composition, root workspace instructions, projection-only tool-result pruning, a DeepSeek Responses adapter, automatic context compaction, local HTTP/SSE, and browser Chat and Trace controls. The [implemented Foundation note](../.agents/notes/implemented/architecture/2026-08-27-execution-foundations.md) owns the Foundation decisions; the [execution service and Web note](../.agents/notes/implemented/architecture/2026-08-27-execution-service-and-web.md) owns the integrated loop, transport, browser, provider, and release-verification decisions; the [Tool Runtime v2 note](../.agents/notes/implemented/architecture/2026-08-31-tool-runtime-v2.md) owns registry and within-run scheduling rationale; the [Context V2 note](../.agents/notes/implemented/architecture/2026-09-01-context-v2.md) owns request composition, workspace instruction admission, and projection-pruning rationale; the [seal state and recovery note](../.agents/notes/implemented/architecture/2026-09-01-seal-state-and-recovery.md) owns bounded derived state and audited blocker-release rationale; and the [project-structure note](../.agents/notes/implemented/architecture/2026-08-31-project-structure-refactor.md) owns the current source-layout and build-boundary rationale.
...
| [server](../packages/server/src/index.ts) | Worker-owned storage, recovery, masking and retention, paged history, approved tools, live agent-loop ownership, request context preparation, root workspace instructions, DeepSeek translation, context compaction, local HTTP/SSE, product startup, and same-origin static delivery | Native database access stays in a Node worker; provider/file/process/network I/O stays in the server; browser code receives only bounded event projections |
| [web](../packages/web/src/App.tsx) | Product Chat mutations, canonical selected-session admission, process-local provider credential controls, and separate Chat and Trace projections | Depends on contracts, not core or server; provider configuration and execution lifecycle authority remain in the server |
| [acceptance](../packages/acceptance/src/foundation-acceptance.ts) | Contributor-only Foundation, controlled-loop, report/viewer, and live-release verification | Depends on public runtime package APIs; no product runtime package depends on acceptance or Playwright |
...
The [event-store reference](event-store.md) owns the asynchronous worker interface, transactions, payload storage, receipts, paging, request limits, bounded replay-derived cache, and exclusive ownership. The [recovery reference](recovery.md) owns startup admission, workspace uncertainty, and audited operator resolution. The worker uses shared schemas and the core reducer inside its persistence boundary; the event ledger remains the only persistent state authority. Its command handler accepts user intent but does not drive effects. Recovery never automatically resumes interrupted work or establishes exactly-once external effects.

## Tool execution boundary

The [tool-service reference](tool-execution.md) owns immutable registry composition, shared approval, conservative scheduling classification, cancellation observation, and durable dispatch. The [file-tool reference](file-tools.md) owns repository discovery, direct-file guards, and managed-mutation evidence; the [shell-tool reference](shell-tools.md) owns bounded output and live process cleanup. Request assembly and dispatch consume the same registered definitions. The service consumes persisted declarations and commits dispatch before I/O; safe sibling bodies may overlap, while terminal events remain in declaration order. Replay and the storage worker never execute tools. This service is not an operating-system sandbox, runtime plugin host, or complete coding-agent loop.
