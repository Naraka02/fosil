...
This reference owns the implemented system composition and dependency boundaries. The repository contains shared contracts, pure state reduction, durable event storage and command acceptance, an immutable built-in tool registry, approved repository/file and shell tools, a provider-neutral agent loop with bounded safe sibling scheduling, a DeepSeek Responses adapter, automatic context compaction, local HTTP/SSE, and browser Chat and Trace controls. The [implemented Foundation note](../.agents/notes/implemented/architecture/2026-08-27-execution-foundations.md) owns the Foundation decisions; the [execution service and Web note](../.agents/notes/implemented/architecture/2026-08-27-execution-service-and-web.md) owns the integrated loop, transport, browser, provider, and release-verification decisions; the [Tool Runtime v2 note](../.agents/notes/implemented/architecture/2026-08-31-tool-runtime-v2.md) owns registry and within-run scheduling rationale; and the [project-structure note](../.agents/notes/implemented/architecture/2026-08-31-project-structure-refactor.md) owns the current source-layout and build-boundary rationale.
