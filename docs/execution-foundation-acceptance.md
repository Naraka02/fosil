...
The [acceptance tests](../packages/acceptance/src/foundation-acceptance.test.ts) run the real driver and check report escaping and the viewer's read-only route boundary. The [driver](../packages/acceptance/src/foundation-acceptance.ts) owns the executable scenarios. Use the full [verification procedure](development.md#setup-and-verification-procedure) as well; the visible report does not replace reducer, storage-corruption, ownership, or race tests.
