...
The initial live coding probe used Flash through the product HTTP service and SQLite Agent Loop. The subsequent [first-release acceptance](release-acceptance.md) added the real product browser, pending-approval refresh, complete Trace inspection, preserved pre-existing user change, service restart over the same database, identical saved-prefix comparison, and a new turn after reopening. Controlled tests retain deterministic ownership of context overflow, provider failure, persistence failure, denial, timeout, cancellation, and interrupted-run recovery.
