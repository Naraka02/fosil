...
Adapter and loop tests use injected semantic streams and therefore do not spend provider tokens. They verify request serialization, strict stream termination, response normalization, usage, context-error classification, and durable attempt correlation. A successful live DeepSeek coding task remains required before claiming real-provider release acceptance.
