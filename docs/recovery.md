...
An unstarted call does not establish an external effect, and a saved tool result is not discarded merely because the run later needed recovery. Unknown outcomes are treated conservatively for every tool kind because there is no implemented tool policy that proves a particular operation harmless. A cleanup failure also remains blocked even if its run has a terminal event.
