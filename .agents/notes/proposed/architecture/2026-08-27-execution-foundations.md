...
The storage worker serializes durable mutations and rejects busy-session submissions through the [command acceptance boundary](../../../../docs/event-store.md#commands-and-receipts). The shared tool service coordinates file and shell dispatch and results with that boundary; provider execution remains dependent work. Permission must be checked again at dispatch; the reducer's validation is not operating-system confinement or a replacement for the tool runner.

When cancellation is accepted, the service must commit intent, resolve pending approvals as cancelled, stop provider reads, and terminate the owned subprocess group. Report cancelled only after the supported runner confirms cleanup. A cleanup failure must prevent new writes in the affected workspace until resolved; the [workspace uncertainty contract](../../../../docs/recovery.md#workspace-uncertainty) now blocks affected admission, while post-crash cleanup and explicit resolution remain future work. The [shell executor](../../../../docs/shell-tools.md#cleanup-and-outcomes) now verifies live cleanup within its owned session, without claiming hostile-descendant containment or cleanup after backend death. [Node's child-process API](https://nodejs.org/api/child_process.html) supplies process operations and cancellation signals; the executor adds tested ownership checks, bounded output draining, signal escalation, and observed exit on top of that API. Aborting a Promise or receiving an AbortError does not establish process cleanup.
...
The initial loop limit is 32 steps per run. Provider requests retain a proposed 120-second deadline; the [shell contract](../../../../docs/shell-tools.md#invocation-boundary) owns its implemented deadline and cleanup window. Cancellation and timeout yield distinct reasons. No provider or tool retries are automatic in the first implementation. The first provider's advertised context window, output reserve, and preflight token-count method must be settled in its adapter slice; no automatic compaction or guessed cross-provider token budget is implied.

### Confirmation and remaining decisions

The bootstrap establishes TypeScript, Node 24, npm workspaces, shared Zod validation, React/Vite bundling, Vitest, and a better-sqlite3 worker probe on Linux. Fastify serves the separate read-only foundation acceptance viewer but has no product HTTP command or SSE service. Playwright is not a workspace dependency; report browser verification uses an isolated installation. Event reduction, transactional payload/event storage, idempotent command admission, bounded worker requests, and exclusive store ownership are effective. Startup recovery, fixed-prefix paging, and provider-neutral history projection are also effective. Shared file/shell dispatch and persisted permission checks are effective, including bounded shell output and live process cleanup. The file-only entry point remains available without enabling shell execution. Agent-loop limits, expiry timers, post-crash cleanup and blocker resolution, configured-secret masking, shell workspace-change attribution, and browser transport remain proposals for their dependent slices. The first real provider and model remain unselected; credentials are unnecessary for bootstrap. Confirm that provider before its integration slice, and keep API keys out of repository documents and conversation output.

### Checkpoint acceptance boundary

The maintainer designated the foundations through approved shell execution as the Execution Foundation checkpoint. The fixed annotated tag identifies that baseline; the [acceptance procedure](../../../../docs/execution-foundation-acceptance.md) owns reproduction and report interpretation. A controlled driver now verifies real file/process effects, saved approvals, replay, and uncertainty recovery without claiming the dependent agent loop or product browser slices. Generated reports retain the baseline identity and actual source manifest so review corrections remain distinguishable from the checkpoint.

### Confirmed Foundation concurrency decision

The maintainer's bounded extension requires different workspaces to execute concurrently within Execution Foundation, without starting the agent-loop or Web slices. Retain the selected TypeScript runtime, one backend host, and the existing shared SQLite worker/store and tool service. The [cross-workspace concurrency contract](../../../../docs/tool-execution.md#cross-workspace-concurrency) owns the effective scope and failure boundaries. This decision is implemented and verified for that boundary; the enclosing note remains proposed because its later loop, transport, and product acceptance remain unfinished.

No execution-core change or new core-to-host IPC protocol is needed for this extension. The work adds evidence for existing per-session admission and per-operation dispatch, not a scheduler or an unlimited-concurrency promise. Two real workspace processes must be live behind unreleased barriers before independent settlement is accepted as evidence. Approval waiting, cancellation, timeout, ordinary failure, and recovery are checked separately from shared-store failure so execution-state isolation is not misrepresented as host fault isolation. The fixed checkpoint tag remains the baseline; the acceptance report identifies the actual reviewed source tree.

### Foundation phase closeout

Execution Foundation is complete for implementation slices 1 through 5c and the controlled acceptance tooling, as verified below. The closeout delivery includes the Unicode path correction, reproducible report and read-only viewer, and bounded cross-workspace concurrency evidence. The original `execution-foundation` tag remains the baseline; the closeout commit records these reviewed additions without redefining that tag. Generated reports remain local evidence rather than tracked fixtures.

The next dependent slice is the controlled-provider loop, with its acceptance conditions defined below. It is not started by closing Foundation. Product HTTP/SSE, Chat/Trace controls, real-provider selection, and the other stated limitations remain outside this phase. Their unfinished status keeps this broader architecture note proposed; it does not leave the verified Foundation scope open or imply product release acceptance.
...
| 5c. Bounded cross-workspace concurrency | 5b | Share one store and tool service across two disjoint workspaces; prove overlapping live processes, approval independence, scoped cancellation/timeout/failure/recovery, and the shared-store failure limit; expose overlap and independent cancellation in the visible report |
| 6. Loop with a controlled provider | 3-5c | Drive read -> edit -> test -> final response without a browser; exercise limits, provider errors, partial stream cancellation, and failures to persist before dependent dispatch |
...
The verified foundation uses Node 24.20.0/npm 10.9.8 on Linux. The complete suite passes 168 tests, including 79 file/service tests, 19 shell-executor tests, and three acceptance/report/viewer tests, alongside strict type checking, production browser bundling, the native SQLite probe, and documentation link/format review. Bootstrap also verified installation from the lockfile in a clean environment. The [development guide](../../../../docs/development.md#setup-and-verification-procedure) owns reproduction commands and the child-process sandbox requirement; the browser probe is not an interactive product test.
...
The controlled acceptance driver passes seven scenarios with 19 observed checks and 177 saved events. Its report exposes the failing and passing test outputs, complete managed diff, preserved user changes, approval decisions, timeout/cancel cleanup, truncation, an injected lost-terminal-write recovery, and concurrent workspace process identities with independent cancellation and completion. Nine Chromium checks cover report counts, inspection controls, the two-process evidence table and separate session traces, filters, refresh without database or effect-counter changes, narrow-screen layout, inert hostile markup, and absence of page errors or external resource requests. These browser checks used isolated Playwright 1.58.2 and Chromium 149; they are not a committed product browser suite. The full npm dependency audit reports no known advisories at review time, which is not a guarantee of future advisory status.
...
This evidence covers the foundations through bounded shell execution and the controlled acceptance report. It does not establish an agent loop, automatic expiry scheduling, secret masking, blocker resolution, shell file-change attribution, adversarial process containment, post-crash cleanup, unrestricted concurrent-writer isolation, real-provider compatibility, HTTP/SSE, browser Chat/Trace, kill-during-commit behavior, disk/power-loss durability, or large-store performance. The note remains proposed because the full execution contracts and product acceptance are unfinished.
