...
The command vocabulary is implemented without HTTP route names. The remaining read surface must list sessions, page history, inspect retained payloads, and stream events without resuming execution. The current full-history read is sufficient for the storage slice, not the final transport or large-session performance boundary.
...
Store/command verification covers transactional rollback of events, payloads, indexes, and receipts; exact and conflicting retries; concurrent same-session submissions; approval/cancellation admission; full lifecycle readback; request-count and byte limits; corruption and worker failure rejection; same-worker and second-process ownership refusal; and committed receipt readback after killing the owning process. The full suite passes 49 tests on Node 24.20.0/npm 10.9.8; the two process tests require sandbox escalation in this environment to observe child output. These checks do not cover a kill during commit, real disk-full or power-loss faults, effect cleanup, paging, startup recovery, HTTP, or provider behavior. The overall note remains proposed because those dependent contracts and the release acceptance are unfinished.
