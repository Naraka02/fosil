...
The storage worker serializes durable mutations and rejects busy-session submissions through the [command acceptance boundary](../../../../docs/event-store.md#commands-and-receipts). The future service must coordinate effect dispatch and callbacks with that boundary. Permission must be checked again at dispatch; the reducer's validation is not operating-system confinement or a replacement for the tool runner.
...
The bootstrap establishes TypeScript, Node 24, npm workspaces, shared Zod validation, React/Vite bundling, Vitest, and a better-sqlite3 worker probe on Linux. Fastify is a locked dependency but has no implemented HTTP service, and Playwright is not installed. Event reduction, transactional payload/event storage, idempotent command admission, bounded worker requests, and exclusive store ownership are effective. Startup recovery, fixed-prefix paging, and provider-neutral history projection are also effective. Runner limits, tool permission enforcement, expiry timers, validated cleanup and blocker resolution, and browser transport remain proposals for their dependent slices. The first real provider and model remain unselected; credentials are unnecessary for bootstrap. Confirm that provider before its integration slice, and keep API keys out of repository documents and conversation output.
...
Bootstrap verification covers clean installation from the lockfile on Node 24.20.0/npm 10.9.8, strict type checking, browser production bundling, shared-schema rejection tests, and a native SQLite worker using a temporary file database. Storage tests cover batch rollback, sequence continuity, reopening committed records, repeated open/close boundaries, and worker error/exit rejection. The standalone probe reports one appended and read event. Dependency installation requires native addon access, and this WSL shell needs a writable Linux TMPDIR for tests. These bootstrap checks alone do not establish the later storage, service, or product guarantees; dependent slices provide separate evidence.

Event/reducer verification uses the same Node 24 environment and covers complete lifecycle fixtures, runtime schema rejection, command and parent correlation, frozen operation checks, cancellation/approval races, child failure closure, late and duplicate terminal facts, explicit recovery provenance, and immutable replay with authoritative final output and unknown metrics. The browser probe imports the event union without core/server dependencies, and the original SQLite probe remains compatible. This is slice-2 evidence; persistence, effect execution, and product acceptance require separate verification.

Store/command verification covers transactional rollback of events, payloads, indexes, and receipts; exact and conflicting retries; concurrent same-session submissions; approval/cancellation admission; full lifecycle readback; request-count and byte limits; corruption and worker failure rejection; same-worker and second-process ownership refusal; and committed receipt readback after killing the owning process. That slice verified 49 tests on Node 24.20.0/npm 10.9.8. Process tests require sandbox escalation in this environment to observe child output. This evidence concerns durable acceptance, not live provider or process-cleanup behavior.

Recovery/history verification covers every prefix of a complete lifecycle, partial text and tool fragments, multiple declared-call correlation, cancellation intent, immutable planning, stale approvals, repeated startup, fixed history bounds across writes/restart, rejected cursors, multi-session recovery rollback, and workspace uncertainty across sessions and overlapping roots. Controlled file-effect fixtures are killed before dispatch, after dispatch, and after result commit; reopen never repeats the effect and only the uncertain dispatched outcome blocks a new run. The complete suite passes 64 tests on the same Node 24 environment, alongside type checking, browser bundling, and the SQLite probe. These checks do not establish real-provider compatibility, real process-group cleanup, a blocker-resolution workflow, kill-during-commit behavior, disk/power-loss durability, large-store performance, HTTP/SSE, or browser interaction. The note remains proposed because the remaining execution contracts and release acceptance are unfinished.
