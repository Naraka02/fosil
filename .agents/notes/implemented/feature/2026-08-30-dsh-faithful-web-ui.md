...
The full-width Trace ledger is closer to the reference and makes long histories easier to scan. Its inspector becomes transient, adding one close action while preserving all previously documented evidence. The new-conversation draft moves the composer into the central DSH focal surface. The global and per-workspace actions seed one browser-memory draft, directory selection occupies the main surface instead of a modal, and workspace switches update that draft without persisting empty sessions. Selecting history cancels the draft; its first message creates the durable session before submitting the run, while every persisted session workspace remains an immutable recorded fact.

## Verification

`npm run typecheck` and `npm run build` pass. With `TMPDIR=/tmp`, the focused workspace ordering, Chat projection, and production Chromium suites pass 8 tests under Node.js 24. Real Chromium verifies the 304-pixel desktop sidebar, inline draft setup without a modal, zero persistence across repeated workspace switches, draft cancellation through history selection, current-workspace and per-workspace draft actions, first-message session creation, conversation-bottom positioning after returning from Trace, and a 390-pixel viewport without horizontal overflow.

The retained captures below document the initial direct DSH copy before the sidebar was compacted and workspace selection moved inline. Their pixel measurements remain historical comparison evidence rather than current geometry assertions. At 1855 x 921, that initial new-session implementation had a mean absolute RGB-channel difference of 2.89 on a 0-255 scale and 1.79% of pixels differed by more than 20 in at least one channel. The focused browser assertions, not those images, own the current sidebar and creation-flow evidence.

| Initial implementation evidence | Dimensions | SHA-256 |
| --- | --- | --- |
...
The current focused Chromium suite also passes the custom permission menu, Full Access confirmation, approval decisions, refresh reconstruction, Trace inspection, Escape dismissal, and a 390-pixel no-overflow check. The complete repository suite was not rerun for the compact-sidebar and inline-creation refinement.
