...
A local coding agent needs attributable execution, durable command acceptance, and recovery before a provider loop or browser controls can safely drive effects. Without shared identities, lifecycle validation, permission ordering, and process ownership, a retry or restart can repeat operations or present an unknown outcome as success. The [implemented workflow decision](../feature/2026-08-27-local-coding-workflow.md) owns the reference choices and product rationale; Foundation establishes the independently verifiable execution boundary beneath that workflow.

## Decision

This note owns the completed Foundation decisions: the runtime and package bootstrap, shared contracts, pure reduction, worker-owned persistence, recovery, approved file and shell tools, bounded cross-workspace concurrency, and controlled acceptance tooling. The [architecture reference](../../../../docs/architecture.md) owns current composition; subsystem references own detailed contracts. The [implemented execution service and Web note](2026-08-27-execution-service-and-web.md) owns the loop phase, product transport, browser controls, provider integration, and release verification extracted from the original broader proposal. This separation changes document scope, not the selected architecture.
...
The controlled report makes Foundation observable without adding product transport. Browser actions cannot execute tools, and fixture declarations are not provider calls. The [dependent service decision](2026-08-27-execution-service-and-web.md) owns the service and product requirements beyond this Foundation boundary; they are not guarantees of the Foundation layer itself.
