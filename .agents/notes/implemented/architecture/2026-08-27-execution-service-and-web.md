...
Normal retained content stops at a 240 MiB logical session budget, with 16 MiB reserved inside the 256 MiB hard budget for cancellation, recovery, and terminal records. Existing 1 MiB tool-result and normalized-provider-output bounds remain independent; browser fields expose at most 64 KiB with explicit metadata. Reaching the normal budget prevents another provider or tool dispatch and settles the active run with an attributable capacity failure. Compaction records count toward the budget and do not reclaim immutable source events. The first release retains saved sessions without age deletion or an export/delete workflow. It does not add application-layer database encryption; the product launcher restricts a newly created local database to its owning operating-system user.
